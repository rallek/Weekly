 ...
 */
function weeklyResetUploadField(fieldName)
{
 ...
 */
function weeklyResetDateField(fieldName)
{
    if ($(fieldName) != undefined) {
        $(fieldName).value = '';
    }
    if ($(fieldName + 'cal') != undefined) {
        $(fieldName + 'cal').update(Zikula.__('No date set.', 'module_Weekly'));
    }
}
 ...
 */
function weeklyInitDateField(fieldName)
{
    if ($('reset' + fieldName.capitalize() + 'Val') != undefined) {
        $('reset' + fieldName.capitalize() + 'Val').observe('click', function (evt) {
            evt.preventDefault();
            weeklyResetDateField(fieldName);
        }).removeClassName('z-hide');
    }
}
 ...
 */
function weeklyToggleRelatedItemForm(idPrefix)
{
 ...
 */
function weeklyResetRelatedItemForm(idPrefix)
{
    // hide the sub form
    weeklyToggleRelatedItemForm(idPrefix);
 ...
 */
function weeklyCreateWindowInstance(containerElem, useIframe)
{
    var newWindow;
 ...
 */
function weeklyInitInlineWindow(objectType, containerID)
{
    var found, newItem;
    ...
    relationHandler.each(function (relationHandler) {
        ...
        if (relationHandler.prefix === containerID) {
            ...
            relationHandler.windowInstance = weeklyCreateWindowInstance($(containerID), true);
        }
    });
    ...
    if (found === false) {
        ...
        newItem.windowInstance = weeklyCreateWindowInstance($(containerID), true);
 ...
 */
function weeklyRemoveRelatedItem(idPrefix, removeId)
{
 ...
 */
function weeklySelectRelatedItem(objectType, idPrefix, inputField, selectedListItem)
{
    ...
    removeLink = Builder.node('a', {id: elemPrefix + 'Remove', href: 'javascript:weeklyRemoveRelatedItem(\'' + idPrefix + '\', ' + newItemId + ');'}, 'remove');
    ...
    if (includeEditing === true) {
        ...
        $(elemPrefix + 'Edit').observe('click', function (e) {
            weeklyInitInlineWindow(objectType, idPrefix + 'Reference_' + newItemId + 'Edit');
            e.stop();
        });
    }
    ...
    if (itemIds !== '') {
        if ($F(idPrefix + 'Scope') === '0') {
            ...
            itemIdsArr.each(function (existingId) {
                if (existingId) {
                    weeklyRemoveRelatedItem(idPrefix, existingId);
                }
            });
            itemIds = '';
        } else {
    ...
    weeklyResetRelatedItemForm(idPrefix);
}

/**
 * Initialise a relation field section with autocompletion and optional edit capabilities
 */
function weeklyInitRelationItemsForm(objectType, idPrefix, includeEditing)
{
    ...
    acOptions = {
        ...
        afterUpdateElement: function (inputField, selectedListItem) {
            ...
            weeklySelectRelatedItem(objectType, idPrefix, inputField, selectedListItem);
        }
    };
    relationHandler.each(function (relationHandler) {
    ...
    $(idPrefix + 'SelectorDoNew').observe('click', function(e) {
        weeklyInitInlineWindow(objectType, idPrefix + 'SelectorDoNew');
        e.stop();
    });
    ...
    itemIdsArr.each(function (existingId) {
        ...
        if (existingId) {
            ...
            $(elemPrefix).observe('click', function (e) {
                weeklyInitInlineWindow(objectType, elemPrefix);
                e.stop();
            });
        }
    });
 ...
 */
function weeklyCloseWindowFromInside(idPrefix, itemId)
{
        ...
        if (relationHandler['prefix'] === idPrefix) {
            ...
            if (itemId > 0) {
                ...
                if (relationHandler.acInstance !== null) {
                    ...
                    // show a message 
                    Zikula.UI.Alert(Zikula.__('Action has been completed.', 'module_weekly_js'), Zikula.__('Information','module_weekly_js'), {
