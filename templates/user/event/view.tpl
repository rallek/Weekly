...
<div class="weekly-event weekly-view">
    ...
    {include file='user/event/view_quickNav.tpl' all=$all own=$own workflowStateFilter=false}{* see template file for available options *}
    ...
    <table class="z-datatable">
        ...
        <tr>
            ...
            <th id="hEventDay" scope="col" class="z-left">
                {sortlink __linktext='Event day' currentsort=$sort modname='Weekly' type='user' func='view' ot='event' sort='eventDay' sortdir=$sdir all=$all own=$own kind=$kind location=$location leader=$leader organisator=$organisator workflowState=$workflowState eventDay=$eventDay searchterm=$searchterm pageSize=$pageSize active=$active}
            </th>
            <th id="hEventBeginText" scope="col" class="z-left">
                {sortlink __linktext='Event begin text' currentsort=$sort modname='Weekly' type='user' func='view' ot='event' sort='eventBeginText' sortdir=$sdir all=$all own=$own kind=$kind location=$location leader=$leader organisator=$organisator workflowState=$workflowState eventDay=$eventDay searchterm=$searchterm pageSize=$pageSize active=$active}
            </th>
            <th id="hEventDuration" scope="col" class="z-right">
                {sortlink __linktext='Event duration' currentsort=$sort modname='Weekly' type='user' func='view' ot='event' sort='eventDuration' sortdir=$sdir all=$all own=$own kind=$kind location=$location leader=$leader organisator=$organisator workflowState=$workflowState eventDay=$eventDay searchterm=$searchterm pageSize=$pageSize active=$active}
            </th>
            ...
            <th id="hEventText" scope="col" class="z-left">
                {sortlink __linktext='Event text' currentsort=$sort modname='Weekly' type='user' func='view' ot='event' sort='eventText' sortdir=$sdir all=$all own=$own kind=$kind location=$location leader=$leader organisator=$organisator workflowState=$workflowState eventDay=$eventDay searchterm=$searchterm pageSize=$pageSize active=$active}
            </th>
            <th id="hMaxParticipants" scope="col" class="z-right">
                {sortlink __linktext='Max participants' currentsort=$sort modname='Weekly' type='user' func='view' ot='event' sort='maxParticipants' sortdir=$sdir all=$all own=$own kind=$kind location=$location leader=$leader organisator=$organisator workflowState=$workflowState eventDay=$eventDay searchterm=$searchterm pageSize=$pageSize active=$active}
            </th>
            <th id="hCurrentParticipants" scope="col" class="z-right">
                {sortlink __linktext='Current participants' currentsort=$sort modname='Weekly' type='user' func='view' ot='event' sort='currentParticipants' sortdir=$sdir all=$all own=$own kind=$kind location=$location leader=$leader organisator=$organisator workflowState=$workflowState eventDay=$eventDay searchterm=$searchterm pageSize=$pageSize active=$active}
            </th>
            <th id="hActive" scope="col" class="z-center">
                {sortlink __linktext='Active' currentsort=$sort modname='Weekly' type='user' func='view' ot='event' sort='active' sortdir=$sdir all=$all own=$own kind=$kind location=$location leader=$leader organisator=$organisator workflowState=$workflowState eventDay=$eventDay searchterm=$searchterm pageSize=$pageSize active=$active}
            </th>
            <th id="hKind" scope="col" class="z-left">
                {sortlink __linktext='Kind' currentsort=$sort modname='Weekly' type='user' func='view' ot='event' sort='kind' sortdir=$sdir all=$all own=$own kind=$kind location=$location leader=$leader organisator=$organisator workflowState=$workflowState eventDay=$eventDay searchterm=$searchterm pageSize=$pageSize active=$active}
            </th>
            <th id="hLocation" scope="col" class="z-left">
                {sortlink __linktext='Location' currentsort=$sort modname='Weekly' type='user' func='view' ot='event' sort='location' sortdir=$sdir all=$all own=$own kind=$kind location=$location leader=$leader organisator=$organisator workflowState=$workflowState eventDay=$eventDay searchterm=$searchterm pageSize=$pageSize active=$active}
            </th>
            <th id="hLeader" scope="col" class="z-left">
                {sortlink __linktext='Leader' currentsort=$sort modname='Weekly' type='user' func='view' ot='event' sort='leader' sortdir=$sdir all=$all own=$own kind=$kind location=$location leader=$leader organisator=$organisator workflowState=$workflowState eventDay=$eventDay searchterm=$searchterm pageSize=$pageSize active=$active}
            </th>
            <th id="hOrganisator" scope="col" class="z-left">
                {sortlink __linktext='Organisator' currentsort=$sort modname='Weekly' type='user' func='view' ot='event' sort='organisator' sortdir=$sdir all=$all own=$own kind=$kind location=$location leader=$leader organisator=$organisator workflowState=$workflowState eventDay=$eventDay searchterm=$searchterm pageSize=$pageSize active=$active}
            </th>
        ...
        <tr class="{cycle values='z-odd, z-even'}">
            ...
            <td headers="hKind" class="z-left">
                {if isset($event.Kind) && $event.Kind ne null}
                    ...
                    {/strip}</a>
                    <a id="kindItem{$event.id}_rel_{$event.Kind.id}Display" href="{modurl modname='Weekly' type='user' func='display' ot='kind' id=$event.Kind.id theme='Printer' forcelongurl=true}" title="{gt text='Open quick view window'}" class="z-hide">{icon type='view' size='extrasmall' __alt='Quick view'}</a>
            ...
            <td headers="hLocation" class="z-left">
                {if isset($event.Location) && $event.Location ne null}
                    ...
                    {/strip}</a>
                    <a id="locationItem{$event.id}_rel_{$event.Location.id}Display" href="{modurl modname='Weekly' type='user' func='display' ot='location' id=$event.Location.id theme='Printer' forcelongurl=true}" title="{gt text='Open quick view window'}" class="z-hide">{icon type='view' size='extrasmall' __alt='Quick view'}</a>
                    <script type="text/javascript">
                    /* <![CDATA[ */
                        document.observe('dom:loaded', function() {
            ...
            <td headers="hLeader" class="z-left">
                {if isset($event.Leader) && $event.Leader ne null}
                    ...
                    {/strip}</a>
                    <a id="leaderItem{$event.id}_rel_{$event.Leader.id}Display" href="{modurl modname='Weekly' type='user' func='display' ot='leader' id=$event.Leader.id theme='Printer' forcelongurl=true}" title="{gt text='Open quick view window'}" class="z-hide">{icon type='view' size='extrasmall' __alt='Quick view'}</a>
                    <script type="text/javascript">
                    /* <![CDATA[ */
                        document.observe('dom:loaded', function() {
            ...
            <td headers="hOrganisator" class="z-left">
                {if isset($event.Organisator) && $event.Organisator ne null}
                    ...
                    {/strip}</a>
                    <a id="organisatorItem{$event.id}_rel_{$event.Organisator.id}Display" href="{modurl modname='Weekly' type='user' func='display' ot='organisator' id=$event.Organisator.id theme='Printer' forcelongurl=true}" title="{gt text='Open quick view window'}" class="z-hide">{icon type='view' size='extrasmall' __alt='Quick view'}</a>
                    <script type="text/javascript">
                    /* <![CDATA[ */
                        document.observe('dom:loaded', function() {
