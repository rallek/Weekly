{* purpose of this template: kind display view in user area *}
...
<div class="weekly-kind weekly-display with-rightbox">
    ...
    <h2>{$templateTitle|notifyfilters:'weekly.filter_hooks.kind.filter'}{icon id='itemActionsTrigger' type='options' size='extrasmall' __alt='Actions' class='z-pointer z-hide'}</h2>
    ...
    {if !isset($smarty.get.theme) || $smarty.get.theme ne 'Printer'}
        <div class="weekly-rightbox">
            ...
            {if isset($kind.event) && $kind.event ne null}
                {include file='user/event/include_displayItemListMany.tpl' items=$kind.event}
            {/if}
            
            {checkpermission component='Weekly:Kind:' instance="`$kind.id`::" level='ACCESS_EDIT' assign='mayManage'}
            {if $mayManage || (isset($uid) && isset($kind.createdUserId) && $kind.createdUserId eq $uid)}
            <p class="managelink">
                {gt text='Create event' assign='createTitle'}
                <a href="{modurl modname='Weekly' type='user' func='edit' ot='event' kind="`$kind.id`" returnTo='userDisplayKind'}" title="{$createTitle}" class="z-icon-es-add">{$createTitle}</a>
            </p>
            {/if}
        </div>
    {/if}
    ...
    {include file='user/include_standardfields_display.tpl' obj=$kind}
