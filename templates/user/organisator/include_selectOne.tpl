{* purpose of this template: inclusion template for managing related organisator in user area *}
...
{/if}
    ...
    {elseif $displayMode eq 'autocomplete'}
        ...
        {if $allowEditing eq true}
            {modurl modname='Weekly' type='user' func='edit' ot='organisator' forcelongurl=true assign='createLink'}
        {/if}
        ...
        <div class="weekly-relation-leftside">
            {if isset($linkingItem.$alias)}
                {include file='user/organisator/include_selectItemListOne.tpl'  item=$linkingItem.$alias}
            {else}
                {include file='user/organisator/include_selectItemListOne.tpl' }
            {/if}
        </div>
        <br class="z-clearer" />
    {/if}
