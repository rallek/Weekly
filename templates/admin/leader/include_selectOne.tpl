{* purpose of this template: inclusion template for managing related leader in admin area *}
...
{/if}
    ...
    {elseif $displayMode eq 'autocomplete'}
        ...
        {if $allowEditing eq true}
            {modurl modname='Weekly' type='admin' func='edit' ot='leader' assign='createLink'}
        {/if}
        ...
        <div class="weekly-relation-leftside">
            {if isset($linkingItem.$alias)}
                {include file='admin/leader/include_selectItemListOne.tpl'  item=$linkingItem.$alias}
            {else}
                {include file='admin/leader/include_selectItemListOne.tpl' }
            {/if}
        </div>
        <br class="z-clearer" />
    {/if}
