{* purpose of this template: comanagers display view in admin area *}
...
<div class="weekly-comanager weekly-display with-rightbox">
    ...
    <div class="z-admin-content-pagetitle">
        ...
        <h3>{$templateTitle|notifyfilters:'weekly.filter_hooks.comanagers.filter'}{icon id='itemActionsTrigger' type='options' size='extrasmall' __alt='Actions' class='z-pointer z-hide'}</h3>
    </div>
    ...
    {if !isset($smarty.get.theme) || $smarty.get.theme ne 'Printer'}
        <div class="weekly-rightbox">
            ...
            {if isset($comanager.event) && $comanager.event ne null}
                {include file='admin/event/include_displayItemListMany.tpl' items=$comanager.event}
            {/if}
            
            {checkpermission component='Weekly:Comanager:' instance="`$comanager.id`::" level='ACCESS_ADMIN' assign='mayManage'}
            ...
            <p class="managelink">
                ...
                <a href="{modurl modname='Weekly' type='admin' func='edit' ot='event' comanager="`$comanager.id`" returnTo='adminDisplayComanager'}" title="{$createTitle}" class="z-icon-es-add">{$createTitle}</a>
            </p>
            {/if}
        </div>
    {/if}
    ...
    {include file='admin/include_standardfields_display.tpl' obj=$comanager}
