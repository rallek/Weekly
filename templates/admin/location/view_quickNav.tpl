{* purpose of this template: locations view filter form in admin area *}
{checkpermissionblock component='Weekly:Location:' instance='::' level='ACCESS_EDIT'}
{assign var='objectType' value='location'}
<form action="{$modvars.ZConfig.entrypoint|default:'index.php'}" method="get" id="weeklyLocationQuickNavForm" class="weekly-quicknav">
    <fieldset>
        ...
        {if !isset($sorting) || $sorting eq true}
                ...
                <select id="sortBy" name="sort">
                    ...
                    <option value="locationName"{if $sort eq 'locationName'} selected="selected"{/if}>{gt text='Location name'}</option>
                    <option value="locationText"{if $sort eq 'locationText'} selected="selected"{/if}>{gt text='Location text'}</option>
                    <option value="locationPicture"{if $sort eq 'locationPicture'} selected="selected"{/if}>{gt text='Location picture'}</option>
...
/* <![CDATA[ */
    document.observe('dom:loaded', function() {
        weeklyInitQuickNavigation('location', 'admin');
