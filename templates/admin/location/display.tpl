{* purpose of this template: locations display view in admin area *}
...
<div class="weekly-location weekly-display with-rightbox">
    ...
    <div class="z-admin-content-pagetitle">
        ...
        <h3>{$templateTitle|notifyfilters:'weekly.filter_hooks.locations.filter'}{icon id='itemActionsTrigger' type='options' size='extrasmall' __alt='Actions' class='z-pointer z-hide'}</h3>
    </div>
    ...
    {if !isset($smarty.get.theme) || $smarty.get.theme ne 'Printer'}
        <div class="weekly-rightbox">
            ...
            {if isset($location.event) && $location.event ne null}
                {include file='admin/event/include_displayItemListMany.tpl' items=$location.event}
            {/if}
            
            {checkpermission component='Weekly:Location:' instance="`$location.id`::" level='ACCESS_ADMIN' assign='mayManage'}
            {if $mayManage || (isset($uid) && isset($location.createdUserId) && $location.createdUserId eq $uid)}
            <p class="managelink">
                {gt text='Create event' assign='createTitle'}
                <a href="{modurl modname='Weekly' type='admin' func='edit' ot='event' location="`$location.id`" returnTo='adminDisplayLocation'}" title="{$createTitle}" class="z-icon-es-add">{$createTitle}</a>
            </p>
            {/if}
        </div>
    {/if}
    ...
    {include file='admin/include_standardfields_display.tpl' obj=$location}
