{* purpose of this template: kind view filter form in admin area *}
{checkpermissionblock component='Weekly:Kind:' instance='::' level='ACCESS_EDIT'}
{assign var='objectType' value='kind'}
<form action="{$modvars.ZConfig.entrypoint|default:'index.php'}" method="get" id="weeklyKindQuickNavForm" class="weekly-quicknav">
    <fieldset>
        ...
        {if !isset($sorting) || $sorting eq true}
                ...
                <select id="sortBy" name="sort">
                    ...
                    <option value="kindName"{if $sort eq 'kindName'} selected="selected"{/if}>{gt text='Kind name'}</option>
                    <option value="kindText"{if $sort eq 'kindText'} selected="selected"{/if}>{gt text='Kind text'}</option>
                    <option value="kindPicture"{if $sort eq 'kindPicture'} selected="selected"{/if}>{gt text='Kind picture'}</option>
...
/* <![CDATA[ */
    document.observe('dom:loaded', function() {
        weeklyInitQuickNavigation('kind', 'admin');
