 ...
 */
class Weekly_Entity_Repository_Base_Event extends EntityRepository
{
    /**
     * @var string The default sorting field/expression.
     */
    protected $defaultSortingField = 'eventName';
    ...
    public function getDescriptionFieldName()
   {
    ...
    public function getAdditionalTemplateParameters($context = '', $args = array())
    {
        ...
        if ($context == 'controllerAction') {
            ...
            if (in_array($args['action'], array('main', 'view'))) {
                ...
                $templateParameters['workflowStateItems'] = $listHelper->getEntries('event', 'workflowState');
                $templateParameters['eventDayItems'] = $listHelper->getEntries('event', 'eventDay');
    ...
    public function truncateTable()
    {
        ...
        $qb->delete('Weekly_Entity_Event', 'tbl');
        $query = $qb->getQuery();
    ...
    public function updateCreator($userId, $newUserId)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('Weekly_Entity_Event', 'tbl')
     ...
     *
     * @return array|Weekly_Entity_Event retrieved data array or Weekly_Entity_Event instance
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false).
     *
     * @return ArrayCollection collection containing retrieved Weekly_Entity_Event instances
     */
    ...
    public function addCommonViewFilters(QueryBuilder $qb)
    {
        ...
        foreach ($parameters as $k => $v) {
            if ($k == 'catId') {
                 ...
                 */
                $qb = ModUtil::apiFunc('Weekly', 'category', 'buildFilterClauses', array('qb' => $qb, 'ot' => 'event', 'catids' => $v));
            } elseif ($k == 'searchterm') {
                // quick search
                if (!empty($v)) {
    ...
    protected function addSearchFilter(QueryBuilder $qb, $fragment = '')
    {
        ...
        if (!$fragmentIsNumeric) {
            ...
            $where .= 'tbl.eventBegin = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.eventName LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.eventDay = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.eventBeginText LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.eventMessage LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.eventText LIKE \'%' . $fragment . '%\'';
        } else {
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.eventBegin = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            ...
            $where .= 'tbl.eventBeginText LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.eventDuration = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.eventMessage LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.eventText LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.maxParticipants = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.currentParticipants = \'' . $fragment . '\'';
        }
        $where = '(' . $where . ')';
    ...
    public function genericBaseQuery($where = '', $orderBy = '', $useJoins = true, $slimMode = false)
    {
        ...
        if ($slimMode === true) {
            ...
            $selection .= ', tbl.eventDay';
            
            
            $selection .= ', tbl.eventBeginText';
            
            
            $selection .= ', tbl.eventName';
            $useJoins = false;
        }
