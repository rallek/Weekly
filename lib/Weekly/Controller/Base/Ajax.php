 ...
 */
class Weekly_Controller_Base_Ajax extends Zikula_Controller_AbstractAjax
{
    ...
    public function getItemListFinder()
    {
        ...
        if ($this->request->isPost() && $this->request->request->has('ot')) {
            $objectType = $this->request->request->filter('ot', 'event', FILTER_SANITIZE_STRING);
        ...
        $entityClass = 'Weekly_Entity_' . ucfirst($objectType);
    ...
    public function getItemListAutoCompletion()
    {
        ...
        if ((is_array($entities) || is_object($entities)) && count($entities) > 0) {
            ...
            if (!empty($previewFieldName)) {
                $imageHelper = new Weekly_Util_Image($this->serviceManager);
    ...
    public function checkForDuplicate()
    {
        ...
        $objectType = $postData->filter('ot', 'event', FILTER_SANITIZE_STRING);
        $controllerHelper = new Weekly_Util_Controller($this->serviceManager);
